...
export default class ReactRro {
  ...
  public static getLastAcitveTextEditor() {
    const { visibleTextEditors, activeTextEditor } = window;
    // const activeTextEditor = visibleTextEditors.find(
    //   (item) => item.document.uri.fsPath === this.activeTextEditorId
    // );
  ...
  private static async handleFile(fileId: number) {
    ...
    if (activeTextEditor) {
      const { position: importDeclarationPosition } = await getImportInfos(
      ...
      activeTextEditor.edit(async (editBuilder) => {
        ...
        editBuilder.insert(
          ...
          getImportTemplate(name, `./components/${name}`)
        );
